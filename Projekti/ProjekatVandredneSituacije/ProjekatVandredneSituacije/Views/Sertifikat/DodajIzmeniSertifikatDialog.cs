...
{
    private SertifikatBasic _selectedSertifikat;
    ...
    private IList<SertifikatBasic> _listaSertifikata;
    ...
    private void RefreshDataGrid()
    {
        try
        {
            _listaSertifikata = DTOMAnager.VratiSertifikate(_operativniRadnik.JMBG);
    ...
    private void BtnDodajIzmeni_Click(object sender, EventArgs e)
    {
        if (ValidateInput())
        {
            ...
            {
                // Dodavanje
                var noviSertifikat = new SertifikatBasic();
    ...
    private void BtnObrisi_Click(object sender, EventArgs e)
    {
        if (dgvSertifikati.SelectedRows.Count > 0)
        {
            ...
            {
                var selectedSertifikat = dgvSertifikati.SelectedRows[0].DataBoundItem as SertifikatBasic;
    ...
    private void DgvSertifikati_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
    {
        ...
        _selectedSertifikat = dgvSertifikati.Rows[e.RowIndex].DataBoundItem as SertifikatBasic;
        if (_selectedSertifikat != null)
        {
            PopulateFields();
